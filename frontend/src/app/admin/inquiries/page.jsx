...
export default function AdminInquiriesPage() {
  ...
  const bulkDelete = async () => {
    ...
    try {
      ...
      if (checked.includes(selected?.id)) setSelected(null)
      setChecked([])
    } catch (error) {
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {checked.length > 0 && (
            <button
              ...
              className="flex items-center gap-2 bg-red-500 hover:bg-red-600 disabled:bg-red-300 text-white text-[12px] font-medium px-4 py-2 rounded-lg transition-colors"
            >
              {bulkDeleting ? (
                <>
                  <div className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                  Deleting {checked.length}...
                </>
              ) : (
                <>
                  <IconTrash size={14} />
        ...
        <div className="lg:col-span-1 flex flex-col gap-2">
          ...
          {inquiries.length > 0 && (
            <div className="flex items-center gap-3 bg-white border border-slate-200 rounded-xl px-4 py-2.5">
              ...
              >
                {isAllSelected ? (
                  <IconSquareCheckFilled size={16} className="text-blue-500" />
                ) : isPartialSelected ? (
                  <IconSquareCheck size={16} className="text-blue-400" />
                ) : (
            ...
            inquiries.map((inq) => (
              ...
              >
                ...
                <button
                  ...
                  className="flex-shrink-0 pt-4 pl-3"
                >
                  {checked.includes(inq.id) ? (
                ...
                <button
                  ...
                  className="flex-1 text-left p-3 pl-1"
                >
                  <div className="flex items-start justify-between gap-2 mb-1">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className={`text-[13px] ${!inq.is_read ? 'font-medium text-slate-900' : 'text-slate-600'}`}>
                        {inq.name}
                      </span>
                    </div>
                  ...
                  <div className="flex items-center gap-1 mt-1.5">
                    <IconClock size={11} className="text-slate-300" />
                ...
                <button
                  ...
                  className="flex-shrink-0 pt-3 pr-3 text-slate-300 hover:text-red-400 transition-colors disabled:opacity-50"
                >
                  {deleting === inq.id ? (
        ...
        <div className="lg:col-span-2">
          {selected ? (
            <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
              ...
              <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 text-[12px] text-red-400 border border-red-100 px-3 py-1.5 rounded-lg hover:bg-red-50 transition-colors disabled:opacity-50"
                  >
                    <IconTrash size={13} />
                    Delete
                  </button>
              ...
              {/* Contact info */}
              <div className="px-6 py-4 border-b border-slate-100 grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="flex items-center gap-2">
              ...
              <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex gap-3">
                <a
                  ...
                  className="flex items-center gap-2 bg-[#1e3a5f] hover:bg-[#162d4a] text-white text-[13px] font-medium px-4 py-2 rounded-lg transition-colors"
                >
                  <IconMail size={14} />
                  Reply via email
                </a>
                {selected.phone && (
                  <a
                    ...
                    className="flex items-center gap-2 bg-green-500 hover:bg-green-600 text-white text-[13px] font-medium px-4 py-2 rounded-lg transition-colors"
                  >
                    WhatsApp reply
                  </a>
                )}
